...
**Что изменилось:**
- `build_close_ata_ix(owner, mint, token_program)` — новая инструкция SPL CloseAccount: закрывает ATA токена и возвращает rent-exempt лампорты (~0.00204 SOL) на кошелёк.
- `build_sell_transaction` получил параметр `close_ata: bool = False` — при True добавляет close-инструкцию ПОСЛЕ sell. Прокинут через `_sell_inner` / `simulate_sell` / `submit_sell`.
- `close_ata=True` валиден ТОЛЬКО на sell, опустошающем позицию (CloseAccount реверсит непустой аккаунт) — частичные продажи оставляют False.

**Зачем:**
- Аудит комиссий (ответ на вопрос «правильно ли считаем комиссии»): формула бумажного PnL (`core.py`) не учитывает аренду ATA. Real-execution создавал ATA (`build_create_ata_idempotent_ix`), но не закрывал — то есть 0.00204 SOL/токен залочены безвозвратно. Это сопоставимо со средней валовой прибылью v21-эры (+0.00184 SOL/сделку) — «прибыльна или околонуля» упиралось в один этот вопрос.
- Решение (выбор пользователя): не «добавить аренду в формулу как расход», а **сделать аренду реально возвратной** — добавить close-ATA. Тогда формула PnL корректна как есть (аренда не тратится), бот остаётся прибыльным.

**Результат:**
- `build_close_ata_ix` проверен: CloseAccount, 3 аккаунта (ata / dest=owner / owner), program=Token-2022. `build_sell_transaction(close_ata=True)` добавляет ровно одну инструкцию. +2 теста, 43 в `test_execution_pumpfun.py` зелёные.
- Поведенчески no-op для текущего бота: `close_ata` default False, бот бумажный, real-execution в live-путь ещё не подключён. Возможность готова — live-cutover передаст `close_ata=True` на полном выходе.
- Прочие комиссии (по итогу аудита): pump.fun 1% + проскальзывание — корректны; приоритетная комиссия в формуле завышена ~14× (консервативно, не баг); базовая комиссия сети (5000 lamports/нога ≈ 0.000015 SOL/сделку) пропущена, но пренебрежима.

**Откат:** `git revert` коммита — чисто аддитивно (новая функция + опц. параметр default False).
