...
- ⏳ Phase −1B (DB Rich↔Mac sync) — opt 3 (hourly dump) is interim
...
**Опции:**
1. **PostgreSQL streaming replication** — Rich primary, Mac standby. Низкий лаг (≤1s). Mac read-only.
2. **Logical replication** — Mac получает только нужные таблицы (trades, token_scores). Гибче.
3. **Periodic dump+restore** — раз в N часов. Простой но устаревший snapshot.

**Решение TBD.** Стартовое — opt 3 (cron job каждый час), upgrade до opt 1 при первой проблеме с актуальностью.
